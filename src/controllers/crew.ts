import { startOfYear, subYears, format, isThisMonth, startOfMonth, endOfMonth } from 'date-fns';
...
export const getAllRecentCrewV1 = async (c: TCFContext) => {
  ...
  const cosmeticsDetails: { [id: string]: IFECosmeticListing } = crewRewardsData.reduce(
  ...
  const recentCrewInDb = await db.query.FortniteCrewTable.findMany({
    ...
    where: ({ crew_date }, { between }) => between(crew_date, startDate, endDate),
    orderBy: [desc(FortniteCrewTable.crew_date)],
  });

  console.log({ recentCrewInDb });

  const recentCrewData = recentCrewInDb.map((r) => {
    const rewards = r.rewards_id
      .map((id) => cosmeticsDetails[id] || null)
      .filter((c) => c !== null);
    const outfitName =
      rewards.find((r) => r.item_type.id.toLowerCase() === 'outfit')?.name.toUpperCase() || '';
  ...
  const crewResponse = Object.keys(crewByYear)
    .map((year) => ({ year: +year, crew_data: crewByYear[+year] }))
...
export const getCurrentActiveCrewV1 = async (c: TCFContext) => {
  ...
  const currentCrew = await db.query.FortniteCrewTable.findFirst({
    ...
    where: ({ crew_date }, { between }) =>
      between(crew_date, startOfMonth(new Date()), endOfMonth(new Date())),
  });
