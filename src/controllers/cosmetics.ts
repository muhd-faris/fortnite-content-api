...
import { CustomException, formatBrListingResponse, hasValueInTag } from '../helpers';
import {
  IBRImages,
  IBRStyleFE,
  IFECosmeticListing,
...
export const getCosmeticFiltersV1 = async (c: TCFContext) => {
  const brCosmeticTypes = brCosmeticTypesData.map((c) => ({
    group: 'battle_royale',
    value: c.value,
    display_name: c.display_name,
  }));
  const festivalCosmeticTypes = festivalCosmeticTypesData.map((c) => ({
    group: 'festival',
    value: c.value,
    display_name: c.display_name,
  }));
...
export const getRecentlyAddedCosmeticsV1 = async (c: TCFContext) => {
  ...
  const fetchedCosmeticListing = await fetch(
    `${FortniteComBaseUrl}/v2/cosmetics/new?language=${lang}`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
  ...
  const instrumentData: IFECosmeticListing[] = cosmeticListingJson.data.items.instruments.map(
    (d) => ({
      id: d.id,
      name: d.name,
      item_type: {
...
export const getRocketRacingListingV1 = async (c: TCFContext) => {
  ...
  const fetchedCosmeticListing = await fetch(
    `${FortniteComBaseUrl}/v2/cosmetics/cars?language=${lang}`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
...
export const getFestivalListingV1 = async (c: TCFContext) => {
  ...
  const fetchedTrackListing = await fetch(
    `${FortniteComBaseUrl}/v2/cosmetics/tracks?language=${lang}`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
  ...
  const fetchedInstrumentListing = await fetch(
    `${FortniteComBaseUrl}/v2/cosmetics/instruments?language=${lang}`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
...
export const getCosmeticDetailsV1 = async (c: TCFContext) => {
  ...
  const shopHistory =
    data.shopHistory?.sort((a, b) => new Date(b).getTime() - new Date(a).getTime()) ?? [];
  const response = {
